...
module SermepaWebTpv
  class Request < Struct.new(:transaction, :description)
    ...
    def must_options(reference="", secure=true)
      ...
      parameters = merchant_parameters(reference, secure)

      options_hash = {
        'Ds_MerchantParameters' => parameters,
        'Ds_Signature' =>  Signature.signature_256(transaction_number.to_s, merchant_secret_key , parameters),
    ...
    def merchant_parameters(reference="", secure=true)
      Base64.encode64(options_for_signature(reference, secure).to_json)
    end
